...
# ============================================================
# Mission lifecycle capture
# ============================================================

async def capture_mission_transition(
    agent_name: str,
    mission_type: str,
    task_id: str,
    from_state: str,
    to_state: str,
    duration_in_prev_state_sec: float = 0.0,
    guard_evaluated: Optional[str] = None,
    guard_result: Optional[bool] = None,
) -> uuid.UUID:
    """Capture a state machine transition within a mission."""
    return await capture_learning_event(
        agent_name=agent_name,
        ...
        outcome={
            "duration_in_previous_state_sec": duration_in_prev_state_sec,
...
async def capture_mission_complete(
    agent_name: str,
    mission_type: str,
    task_id: str,
    ...
    """Capture a mission reaching its terminal state."""
    return await capture_learning_event(
        agent_name=agent_name,
        ...
        context={
            ...
            "mission_type": mission_type,
            "task_id": task_id,
            "total_transitions": len(transition_path) - 1,
        },
        outcome={
...
async def capture_error_recovery(
    ...
    """Capture an error recovery event (task reset for retry)."""
    return await capture_learning_event(
        agent_name=agent_name,
        event_type="mission_error_recovery",
        context={
            "agent_name": agent_name,
            "mission_type": mission_type,
            "task_id": task_id,
...
async def get_relevant_patterns(
    ...
    When mission_type is provided, only returns patterns for that mission.
    """
    ...
    async with AsyncSessionLocal() as session:
        ...
        if mission_type:
            stmt = stmt.where(
                ...
                | (LearningPattern.mission_type.is_(None))
            )

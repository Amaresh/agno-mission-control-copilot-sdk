        ...
        try:
            async with AsyncSessionLocal() as session:
                ...
                for ag in agents:
                    ...
                    role_tag = "developer"
                    if "doc" in (ag.role or "").lower() or "plan" in (ag.role or "").lower():
                        role_tag = "planner/docs"
                    elif "test" in (ag.role or "").lower() or "qa" in (ag.role or "").lower():
                        role_tag = "testing/QA"
                    elif "review" in (ag.role or "").lower() or ag.level and ag.level.value == "lead":
